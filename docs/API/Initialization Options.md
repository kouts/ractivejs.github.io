...
A map of [adaptors](../Extend/Adaptors.md) where the key is the adaptor name and the value is a reference to an adaptor definition.
...
`adaptors` is not required if you directly specified an adaptor definition via `adapt`.
...
const instance = new Ractive({
  ...
  // No need to register AdaptorDefinition
});
```
...
An `id` of the element, a CSS selector to an element, a reference to an HTML element, or an array-like object whose first item is an HTML element, which is a child of `el` will render the instance before that element.
...
A map of [components](../Extend/Components.md) where the key is the component name the value is either a reference to a component definition or a function that returns either a name of a registered component or a reference to a component definition. The function receives processed [`data`](#data) as first argument.
...
Models can also be used to supply data. Ractive will treat them like POJOs which means it can only read and write to publicly visible properties.
...
If you use a constructor guard clause for your model, you can simply provide the reference to the model directly instead using a data function.
...
A map of [decorators](../Extend/Decorators.md) where the key is the decorator name and the value is a reference to a decorator definition.
...
The element to render an instance to. Can either be an `id` of the element, a CSS selector to an element, a reference to an HTML element, or an array-like object whose first item is an HTML element.
...
A map of [events](../Extend/Events.md) where the key is the event name and value is a reference to an event definition.
...
A map of [interpolators](../Extend/Interpolators.md) where the key is the interpolator name and the value is a reference to an interpolator definition.
...
An object is also accepted. The object form should have `elements` which is an array of blacklisted elements and `eventAttributes` boolean which, when `true`, also strips out event attributes.
